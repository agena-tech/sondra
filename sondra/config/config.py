...
class Config:
    """Configuration Manager for Sondra."""
    ...
    sondra_llm_max_retries = "5"
    sondra_memory_compressor_timeout = "30"
    llm_timeout = "300"
    _LLM_CANONICAL_NAMES = (
        ...
        "litellm_base_url",
        "ollama_api_base",
        "sondra_reasoning_effort",
        "sondra_llm_max_retries",
        "sondra_memory_compressor_timeout",
        "llm_timeout",
    )

    # Tool & Feature Configuration
    perplexity_api_key = None
    sondra_disable_browser = "false"

    # Runtime Configuration
    sondra_image = "ghcr.io/usestrix/strix-sandbox:0.1.13"
    sondra_runtime_backend = "docker"
    sondra_sandbox_execution_timeout = "120"
    sondra_sandbox_connect_timeout = "10"

    # Telemetry
    sondra_telemetry = "1"
    sondra_otel_telemetry = None
    sondra_posthog_telemetry = None
    ...
    def tracked_vars(cls) -> list[str]:
        names: list[str] = []
        for name in cls._tracked_names():
            for env_name in cls._env_names_for_config_name(name):
    ...
    def _llm_env_vars(cls) -> set[str]:
        names: set[str] = set()
        for name in cls._LLM_CANONICAL_NAMES:
            names.update(cls._env_names_for_config_name(name))
        return names

    @staticmethod
    def _sondra_env_name(env_name: str) -> str:
        normalized = str(env_name or "").strip().upper()
        return normalized

    @staticmethod
    def _sondra_config_name(name: str) -> str:
        return str(name or "").strip().lower()

    @classmethod
    def _env_names_for_config_name(cls, name: str) -> list[str]:
        canonical_name = cls._sondra_config_name(name)
        primary_name = cls._sondra_env_name(canonical_name.upper())
        return [primary_name]
    ...
    def get(cls, name: str) -> str | None:
        canonical_name = cls._sondra_config_name(name)
        default = getattr(cls, canonical_name, getattr(cls, name, None))
        for env_name in cls._env_names_for_config_name(canonical_name):
