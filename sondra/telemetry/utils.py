...
class _SecretTokenDetector(RegexDetector):  # type: ignore[misc]
    name = "sondra_secret_token_detector"
...
def default_resource_attributes() -> dict[str, str]:
    return {
        "service.name": "sondra-agent",
        "service.namespace": "sondra",
    }
...
def prune_otel_span_attributes(attributes: dict[str, Any]) -> dict[str, Any]:
    ...
    if filtered_count:
        filtered["sondra.filtered_attributes_count"] = filtered_count
...
class JsonlSpanExporter(SpanExporter):  # type: ignore[misc]
    ...
    def export(self, spans: Sequence[ReadableSpan]) -> SpanExportResult:
        ...
        for span in spans:
            ...
            if "sondra.event_type" in attributes:
    ...
    def _span_to_record(
        ...
        run_id_attr = (
            attributes.get("sondra.run_id")
        or attributes.get("sondra_run_id")
            or run_metadata.get("run_id")
            or span.resource.attributes.get("sondra.run_id")
        )
...
def bootstrap_otel(
    *,
    ...
    sanitizer: Callable[[Any], Any],
    write_lock_getter: Callable[[Path], threading.Lock],
    tracer_name: str = "sondra.telemetry.tracer",
