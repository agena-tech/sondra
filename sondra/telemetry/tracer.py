...
try:
    ...
    trace = _DummyTraceModule()  # type: ignore[assignment]

from sondra.config import Config
from sondra.telemetry import posthog
from sondra.telemetry.flags import is_otel_enabled
from sondra.telemetry.utils import (
    TelemetrySanitizer,
    append_jsonl_record,
    bootstrap_otel,
    format_span_id,
...
class Tracer:
    ...
    def _setup_telemetry(self) -> None:
        ...
        (
            ...
            tracer_name="sondra.telemetry.tracer",
        )
        ...
        if self._otel_tracer is not None:
            try:
                with self._otel_tracer.start_as_current_span(
                    ...
                    span.set_attribute("sondra.event_type", event_type)
                    span.set_attribute("sondra.source", source)
                    span.set_attribute("sondra.run_id", self.run_id)
                    span.set_attribute("sondra.run_name", self.run_name or "")

                    if status:
                        span.set_attribute("sondra.status", status)
    ...
    def save_run_data(self, mark_complete: bool = False) -> None:
        try:
            ...
            if self.final_scan_result:
                report_file = run_dir / "security_report.md"
                ...
                logger.info(
                    "Saved final security report to: %s",
                    report_file,
                )
