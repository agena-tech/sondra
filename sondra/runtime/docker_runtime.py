...
class DockerRuntime(AbstractRuntime):
    ...
    def _create_container(self, scan_id: str, max_retries: int = 2) -> Container:
        container_name = f"sondra-scan-{scan_id}"
        image_name = Config.get("sondra_image")
        if not image_name:
            raise ValueError("SONDRA_IMAGE must be configured")
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                execution_timeout = Config.get("sondra_sandbox_execution_timeout") or "120"
    ...
    def _get_or_create_container(self, scan_id: str) -> Container:
        ...
        try:
            containers = self.client.containers.list(
                all=True, filters={"label": f"sondra-scan-id={scan_id}"}
            )
            if containers:
