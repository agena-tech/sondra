...
def _is_http_git_repo(url: str) -> bool:
    ...
    try:
        req = Request(check_url, headers={"User-Agent": "git/sondra"})  # noqa: S310
...
def clone_repository(repo_url: str, run_name: str, dest_name: str | None = None) -> str:
    ...
    temp_dir = Path(tempfile.gettempdir()) / "sondra_repos" / run_name
    ...
    try:
        ...
        error_text.append(
            f"Error: {e.stderr if hasattr(e, 'stderr') and e.stderr else str(e)}", style="red"
        )
...
def check_docker_connection() -> Any:
    try:
        ...
        error_text.append(
            "Please ensure Docker Desktop is installed and running, and try running sondra again.\n",
            style="bold white",
        )
