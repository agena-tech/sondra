...
class BrowserRenderer(BaseToolRenderer):
    ...
    SIMPLE_ACTIONS: ClassVar[dict[str, str]] = {
        ...
        "screenshot": "capturing browser image",
        "wait": "waiting...",
    ...
    def _build_url_action(cls, text: Text, label: str, url: str | None, suffix: str = "") -> None:
        text.append(label, style="#00FCC1")
        if url:
            text.append(url, style="#00FCC1")
            if suffix:
                text.append(suffix, style="#00FCC1")
    ...
    def _build_content(cls, action: str, args: dict[str, Any]) -> Text:
        ...
        if action in cls.SIMPLE_ACTIONS:
            text.append(cls.SIMPLE_ACTIONS[action], style="#00FCC1")
            return text
        ...
        if action in url_actions:
            ...
            if action == "launch" and not url:
                text.append("launching browser", style="#00FCC1")
            else:
        ...
        if action in click_actions:
            text.append(click_actions[action], style="#00FCC1")
            return text
        ...
        if action in handlers:
            ...
            text.append(label, style="#00FCC1")
            if value:
                text.append(str(value), style="#00FCC1")
            return text

        if action == "execute_js":
            text.append("executing javascript", style="#00FCC1")
        ...
        if action:
            text.append(action, style="#00FCC1")
        return text
