...
PROMPT_PREFIX_RE = re.compile(r"^\[(?:SONDRA|SONDRA)_\d+\]\$?\s*")
...
class TerminalRenderer(BaseToolRenderer):
    ...
    def _clean_output(cls, output: str, command: str = "") -> str:
        ...
        if cleaned.strip():
            ...
            for line in lines:
                normalized_line = PROMPT_PREFIX_RE.sub("", line, count=1)

                if not filtered_lines and not normalized_line.strip():
                    continue
                if not normalized_line.strip():
                    continue
                if command and normalized_line.strip() == command.strip():
                    continue
                if command and re.match(
                    r"^[\$#>]\s*" + re.escape(command.strip()) + r"\s*$", normalized_line
                ):
                    continue
                filtered_lines.append(normalized_line)

            while filtered_lines and not PROMPT_PREFIX_RE.sub(
                "", filtered_lines[-1], count=1
            ).strip():
                filtered_lines.pop()
