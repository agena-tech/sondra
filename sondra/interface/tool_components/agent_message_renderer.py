...
def _is_status_line(line: str) -> bool:
    ...
    if not normalized:
        return False
    # Accept lines even if terminals replace emojis/symbols with fallback glyphs.
    if "görev eklendi" in normalized:
        return True
    if "tasks cleared" in normalized or "görevleri başarıyla sildim" in normalized:
        return True
    if normalized.startswith("○ waiting") or normalized.startswith("o waiting"):
        return True
    if normalized.startswith("waiting for the scheduled time"):
        return True
    return False
...
def _apply_markdown_styles(text: str) -> Text:  # noqa: PLR0912
    ...
    for i, line in enumerate(lines):
        ...
        if header:
            ...
            result.append(line, style="#8a8a8a")
        else:
