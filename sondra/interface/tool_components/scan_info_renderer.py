...
class ScanStartInfoRenderer(BaseToolRenderer):
    ...
    def render(cls, tool_data: dict[str, Any]) -> Static:
        ...
        scan_mode = str(args.get("scan_mode", "general")).lower()
        scan_level = str(args.get("scan_level", "standard")).lower()
        model_name = cls._format_model_name(str(args.get("model_name", "")))

        text = Text()
        text.append("◈ ", style="#22c55e")
        text.append(cls._build_start_text(scan_mode, scan_level, model_name))
    ...
    @classmethod
    def _format_model_name(cls, raw_name: str) -> str:
        if not raw_name:
            return "MODEL"
    ...
    @classmethod
    def _build_start_text(cls, scan_mode: str, scan_level: str, model_name: str) -> str:
        if scan_mode == "general":
            return f"Starting {model_name} LLM ..."
        if scan_mode == "adb":
            return f"Starting {model_name} ADB control mode ..."
        if scan_mode == "osint":
            return "Starting osint test ..."
