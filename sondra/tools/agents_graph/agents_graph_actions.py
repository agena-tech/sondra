...
def _get_root_fixed_agents(agent_state: Any, parent_agent: Any | None) -> int | None:
    direct_fixed_agents = getattr(getattr(parent_agent, "llm_config", None), "fixed_agents", None)
    if direct_fixed_agents is not None:
        return direct_fixed_agents

    context = getattr(agent_state, "context", {}) or {}
    inherited_fixed_agents = context.get("root_fixed_agents")
    if inherited_fixed_agents is None:
        return None

    try:
        return int(inherited_fixed_agents)
...
def _run_agent_in_thread(
    ...
    try:
        ...
        fixed_subagent_instruction = ""
        context = getattr(state, "context", {}) or {}
        if context.get("fixed_subagent_mode"):
            fixed_subagent_instruction = (
                "\n        - Fixed sub-agent mode is active for this run"
                "\n        - You are a worker, not an orchestrator"
                "\n        - Do NOT call create_agent"
                "\n        - Complete your assigned task yourself and return results with agent_finish"
            )
...
def create_agent(
    ...
    try:
        ...
        from sondra.skills import parse_skill_list, validate_requested_skills
        ...
        from sondra.agents import SondraAgent
        ...
        if parent_agent and hasattr(parent_agent, "llm_config"):
            ...
            if hasattr(parent_agent.llm_config, "scan_level"):
                scan_level = parent_agent.llm_config.scan_level
            if hasattr(parent_agent.llm_config, "retry_attempts"):
                retry_attempts = parent_agent.llm_config.retry_attempts

        root_fixed_agents = _get_root_fixed_agents(agent_state, parent_agent)

        if root_fixed_agents is not None and getattr(agent_state, "parent_id", None) is not None:
            return {
                "success": False,
                "error": (
                    "Fixed sub-agent mode is active. Only the root agent may create sub-agents; "
                    "complete the delegated task yourself and report back with agent_finish."
                ),
                "agent_id": None,
            }

        if root_fixed_agents is not None and getattr(agent_state, "parent_id", None) is None:
            direct_children = sum(
                1 for node in _agent_graph["nodes"].values() if node.get("parent_id") == parent_id
            )
            if direct_children >= root_fixed_agents:
                return {
                    "success": False,
                    "error": (
                        f"Fixed sub-agent limit reached: {direct_children}/"
                        f"{root_fixed_agents} direct root sub-agents already exist"
                    ),
                    "agent_id": None,
                }
        ...
        if root_fixed_agents is not None:
            state.update_context("root_fixed_agents", root_fixed_agents)
            state.update_context("fixed_subagent_mode", True)
        ...
        agent = SondraAgent(agent_config)
        if parent_agent and hasattr(parent_agent, "memory_store"):
            agent.memory_store = parent_agent.memory_store
