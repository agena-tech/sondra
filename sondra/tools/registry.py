...
def _process_dynamic_content(content: str) -> str:
    if "{{DYNAMIC_SKILLS_DESCRIPTION}}" in content:
        try:
            from sondra.skills import generate_skills_description
...
def _get_schema_path(func: Callable[..., Any]) -> Path | None:
    ...
    return get_sondra_resource_path("tools", folder, schema_file)
...
def get_tools_prompt(exclude_modules: set[str] | None = None) -> str:
    excluded = {m.lower() for m in (exclude_modules or set())}
    tools_by_module: dict[str, list[dict[str, Any]]] = {}
    for tool in tools:
        module = tool.get("module", "unknown")
        if str(module).lower() in excluded:
            continue
